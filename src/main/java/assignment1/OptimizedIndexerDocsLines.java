...
public class OptimizedIndexerDocsLines {
  ...
  public static class TokenizerMapper extends Mapper<Object, Text, IndexKey, Text>{
    ...
    private Map<String, StringBuffer> inDocumentOccurences = new HashMap<String, StringBuffer>();
    private String id = "";
    private String text = "";
    ...
    public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
      ...
      terms.clear();
      this.inDocumentOccurences.clear();
      tokenPosition = 0;
    	  ...
    	  if(next.length() > 2 && (this.blockParsingTranslations || (next.indexOf(":") != -1 && next.substring(0, 2).equals("[[")))){
        	  ...
        	  if(!skip){
        		  if(terms.containsKey(next.toString())){
        			  ...
        			  StringBuffer temp = this.inDocumentOccurences.get(next.toString());
        			  temp.append(" ").append("" + tokenPosition);
        			  this.inDocumentOccurences.put(next.toString(), temp);
        		  }
        		  else{
        			  terms.put(next.toString(), 1);
        			  this.inDocumentOccurences.put(next.toString(), new StringBuffer("" + tokenPosition));
        		  }
        	  }
    	  }
      ...
      for (Map.Entry<String, Integer> entry : terms.entrySet()) {
    	    k = entry.getKey();
    	    v = entry.getValue().intValue() + " " + this.inDocumentOccurences.get(k).toString();
...
public static class IndexReducer extends Reducer<IndexKey,Text,Text,Text> {
	...
	public void close(Context context) throws IOException, InterruptedException{
		context.write(new Text(last), new Text(postings.toString()));
	}
}
...
//    FileOutputFormat.setOutputPath(job, new Path("s3://in4325-enwiki/output/be_ka_zi/docindex/"));
    ...
    job.setJarByClass(OptimizedIndexerDocsLines.class);
